# Automatic Maintenance of Oracle Archive Log Files for Oracle Database 19c (ORCLDB) on Windows

This document describes the archive log maintenance procedure using the RMAN script `maint_archive_log.rman` together with the wrapper batch file `maint_archive_log.bat` (version 1.0) for Oracle Database 19c (container database) running on Microsoft Windows. The script synchronizes archive log metadata with the physical files on disk, removes expired metadata entries, and deletes archive log files older than a configurable retention period (default: 7 days) using RMAN. Execution is logged to a timestamped `.log` file and can be automated with Windows Task Scheduler.
...
flowchart TD
    ...
    I -- yes --> K[Query FRA usage + archive log<br/>distribution by day]
    K --> L[Write END entry to log file]
    L --> M([Exit - Maintenance completed])
```
...
sequenceDiagram
    ...
    BAT->>CDB: query FRA usage + archive log distribution by day (sqlplus)
    CDB-->>BAT: space used/limit, log counts, oldest log
...
- **Enhanced FRA monitoring** — after RMAN cleanup, the `.bat` script queries FRA space used/limit and archive log distribution by day via `sqlplus`
...
-- Archive log distribution by date
SELECT TO_CHAR(FIRST_TIME, 'YYYY-MM-DD') AS LOG_DATE, COUNT(*) AS COUNT
...
| `SPACE_USED_MB` / `SPACE_LIMIT_MB` / `SPACE_RECLAIMABLE_MB` | FRA usage statistics in MB |
| `PERCENT_USED` | Percentage of FRA capacity currently used |
| `LOG_DATE` / `COUNT` | Archive log count grouped by day |
...
Format: `[START]/[END] tag, date/time, RMAN command output, FRA monitoring output`
...
- This 19c variant adds enhanced FRA monitoring (space usage and archive log distribution by day) after RMAN cleanup completes, compared to the simpler FRA space check used in the 11g/12c Windows variants.
...
| FRA monitoring query fails    | Verify `sqlplus` connectivity and SYSDBA privileges independently of RMAN |
...
| `rman` executable not found  | Verify `ORACLE_HOME\bin` is on `PATH` and points to a valid 19c install |
...
- Oracle `sysdba` access at the CDB root level for RMAN and `sqlplus` operations
...
REM               plus enhanced FRA monitoring
REM Database    : Oracle 19c on Microsoft Windows
...
echo Checking Flash Recovery Area usage and archive log distribution... >> <LOG_DIRECTORY>\arch_maint.log

sqlplus -s / as sysdba << EOF >> <LOG_DIRECTORY>\arch_maint.log
...
FROM V\$RECOVERY_FILE_DEST;
PROMPT
PROMPT === Archive Log Count by Day ===
SELECT
