# Automatic Maintenance of Oracle Archive Log Files for Oracle Database 19c (ORCLDB) on Oracle Linux 8

This document describes the archive log maintenance script `maint_archivelog_oradb19c.sh` (version 1.0) for Oracle Database 19c running on Oracle Linux 8. The script synchronizes archive log metadata with the physical files on disk, removes expired metadata entries, and deletes archive log files older than a configurable retention period (default: 7 days) using RMAN. Execution is logged to a timestamped `.log` file.
...
| File Name      | `maint_archivelog_oradb19c.sh`                    |
| Version        | 1.0                                               |
...
flowchart TD
    ...
    I -- yes --> J[Query CDB/PDB archive log + FRA inventory]
...
sequenceDiagram
    ...
    S->>CDB: SELECT archive log distribution by date
    CDB-->>S: log counts, oldest log, total size
...
chmod +x maint_archivelog_oradb19c.sh

# Manual execution
./maint_archivelog_oradb19c.sh

# Automated via cron (weekly, Sunday at 2:00 AM)
0 2 * * 0 /path/to/maint_archivelog_oradb19c.sh
...
|----------------------------|---------------------------------------------------------------|--------------------------------------------------|
| `ORACLE_HOME`               | `/u01/app/oracle/product/19c/dbhome_1`                         | Oracle software home                              |
...
- **Enhanced FRA monitoring** — reports FRA space used/limit, archive log distribution by day, and oldest archive log
- **CDB/PDB-aware** — archive log destination and FRA usage are managed at the CDB (root container) level, shared across all pluggable databases
...
-- Oldest archive log
SELECT TO_CHAR(FIRST_TIME, 'YYYY-MM-DD HH24:MI:SS') AS FIRST_TIME, NAME
FROM V$ARCHIVED_LOG WHERE DELETED = 'NO'
AND FIRST_TIME = (SELECT MIN(FIRST_TIME) FROM V$ARCHIVED_LOG WHERE DELETED = 'NO');
...
- Compare against the companion cleanup script (`oracle-19c-cleanup-archivelog-script-ol8.md`) for a variant with filesystem-level FRA directory/archive cleanup enabled by default; this maintenance script is intentionally more conservative.
...
## Script File: maint_archivelog_oradb19c.sh
...
# Database    : Oracle 19c (CDB) on Oracle Linux 8
...
#                   - Enhanced FRA monitoring (distribution by day, oldest log)
...
# Enhanced FRA monitoring (CDB root)
log_message "Checking Flash Recovery Area usage and archive log distribution..."
